...
SELECT
    ...
    tf."Funder ID",
    sum(sf.value) AS "Total disb. to int'l orgs. post-2013 for cap. (nom. USD)"
FROM
    top_funders tf
...
WHERE
    ...
    AND target.sankey_cat = 'International'
    AND tf."Funder ID" IN (
        SELECT
            *
        FROM
...
GROUP BY
    ...
    tf."Funder ID"
ORDER BY
    "Total disb. to int'l orgs. post-2013 for cap. (nom. USD)" DESC